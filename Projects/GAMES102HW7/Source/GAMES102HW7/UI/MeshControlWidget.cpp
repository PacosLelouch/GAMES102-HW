...
void UMeshControlWidget::BindEventsRuntime()
{
	...
	CheckBox_Soft->OnCheckStateChanged.AddDynamic(this, &UMeshControlWidget::ChangeCheckBoxSoftState);
...
void UMeshControlWidget::BindValuesRuntime()
{
	...
	if (Controller)
	{
		CheckBox_Soft->SetIsChecked(Controller->bSoft);
	}
...
FText UMeshControlWidget::GetTextSoft()
{
	static FText Text = FText::FromString("Soft");
	return Text;
}
...
void UMeshControlWidget::ChangeCheckBoxSoftState(bool bIsChecked)
{
	if (Controller) {
		Controller->bSoft = bIsChecked;
	}
}
...
void UMeshControlWidget::MakeAssetGridPanel(UUniformGridPanel* UniformGridPanel, const TArray<FAssetData>& Assets, TSubclassOf<UAssetEntryWidget> EntryWidgetClass)
{
	...
	for (int32 i = 0; i < Assets.Num(); ++i)
	{
		...
		UAssetEntryWidget* Entry = CreateWidget<UAssetEntryWidget>(UniformGridPanel, EntryWidgetClass);
