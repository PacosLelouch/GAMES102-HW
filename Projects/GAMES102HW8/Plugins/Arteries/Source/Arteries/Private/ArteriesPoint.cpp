...
TArray<FArteriesLink*> FArteriesPoint::GetSortedLinks(bool bContainForward, bool bContainBackward)
{
	TArray<FArteriesLink*> OutLinks;
	for (FArteriesLink& L : Links)
	{
		...
		if (bContainForward)
		{
			...
			while (true)
			{
				...
				if (!Link || OutLinks.Contains(Link))
					break;
				//if((bContainForward && !Link->Inverse) || (bContainBackward && Link->Inverse))
