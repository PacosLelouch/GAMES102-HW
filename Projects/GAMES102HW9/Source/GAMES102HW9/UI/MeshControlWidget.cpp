...
void UMeshControlWidget::BindValues()
{
	...
	Text_SimpRatio->SetText(GetTextSimpRatio());
	Text_LoopCount->SetText(GetTextLoopCount());
	Text_Process->SetText(GetTextProcess());

	SpinBox_SimpRatio->SetMinValue(0.0);
	SpinBox_SimpRatio->SetMaxValue(1.0);
	SpinBox_SimpRatio->SetMinSliderValue(0.0);
	SpinBox_SimpRatio->SetMaxSliderValue(1.0);
	SpinBox_SimpRatio->Delta = 1. / 256.;
	SpinBox_SimpRatio->SliderExponent = 1;
...
{
	SpinBox_SimpRatio->OnValueCommitted.AddDynamic(this, &UMeshControlWidget::CommitSpinBoxSimpRatio);
	...
	if (Controller)
	{
		Controller->OnProceduralHandlerValidityChanged.BindLambda([this]()
		{
			...
			SpinBox_SimpRatio->SetIsEnabled(bEnabledOperation);
		});
	}
}
...
{
	BindValues();
	SpinBox_SimpRatio->SetValue(GetSpinBoxSimpRatio());
	SpinBox_LoopCount->SetValue(GetSpinBoxLoopCount());
	//SpinBox_SimpRatio->ValueDelegate.BindDynamic(this, &UMeshControlWidget::GetSpinBoxSimpRatio);
...
FText UMeshControlWidget::GetTextSimpRatio()
{
	static FText Text = FText::FromString("SimpRatio");
	return Text;
}
...
float UMeshControlWidget::GetSpinBoxSimpRatio()
{
	if (Controller) {
		return Controller->InputParams.SimpRatio;
	}
	return 0.f;
}
...
void UMeshControlWidget::CommitSpinBoxSimpRatio(float InValue, ETextCommit::Type CommitMethod)
{
	if (Controller) {
		Controller->InputParams.SimpRatio = InValue;
