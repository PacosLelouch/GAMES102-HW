...
void FArteriesPrimitive::Build(FProcMeshSection& Section, TMap<FArteriesPoint*, int32>& VerticesMapper, TArray<FArteriesPrimitive*>* Holes)
{
	...
	auto CreateVertex = [&](FArteriesPoint* Point, bool bHole)->int
	{
		...
		// Find Previous
		int32* IndexPtr = VerticesMapper.Find(Point);
		if (IndexPtr)
		{
			return *IndexPtr;
		}
		...
		// Find Previous
		VerticesMapper.Add(Point, Index);
		...
		{
			Vert.UV2 = Point->GetVec2(AVN_UV2);
		}
		if (Point->HasVec2(AVN_UV3))
		{
			Vert.UV3 = Point->GetVec2(AVN_UV3);
		}
		if (Point->HasVec3(AVN_Color))
		{
			Vert.Color = FLinearColor(Point->GetVec3(AVN_Color)).ToFColor(true);
		}
		if (HasInt(AVN_Stencil))
		{
			Vert.UV3.Y = static_cast<float>(GetInt(AVN_Stencil));
		}
